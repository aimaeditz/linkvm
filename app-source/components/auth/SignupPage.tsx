...
import { User, Mail, Lock, Eye, EyeOff, ArrowRight, Check, AlertCircle, Sparkles } from 'lucide-react';
...
export const SignupPage: React.FC<SignupPageProps> = ({ onSuccess, onNavigate }) => {
    ...
    if (refParam) {
      ...
      try {
        ...
        const found = users.find(
          (u: any) => u.referralCode === refParam || u.username.toLowerCase() === refParam.toLowerCase() || u.username.toLowerCase() === refParam.split('-')[0]
        );
  ...
  const handleSuccess = () => {
    // Record login timestamp for welcome messages
